// Copy From https://cs.opensource.google/go/go/+/refs/tags/go1.19.3:src/flag/flag.go
...
京城郭少的笔记
原理
	flag会将参数(变量名)注册到全局变量map的key中，调用flag.Parse()时会将变量值保存到全局变量map的value中
*/
...
// bool变量会保存到p指针指向的空间。The argument p points to a bool variable in which to store the value of the flag.
// p：bool变量的地址
// name：参数名
// value：默认值
// usage：描述
...
// 返回值一个地址，这个地址存储的是布尔变量。The return value is the address of a bool variable that stores the value of the flag.
// name：参数名
// value：默认值
// usage：描述
...
func (f *FlagSet) parseOne() (bool, error) {
	...
	//拿到第一个参数(可能是"-n  --n  n各种格式")
	s := f.args[0]
	//参数校验。如果是'-'或非'-'开头的，直接返回
	if len(s) < 2 || s[0] != '-' {
		return false, nil
	}
	//判断是'-'类型还是'--'类型
...
func (f *FlagSet) Parse(arguments []string) error {
	f.parsed = true    //表示已解析
	f.args = arguments //将参数保存到全局变量中
	for {
		seen, err := f.parseOne() //尝试解析一个KV类参数
		if seen {
			continue //如果解析成功了，继续解析
		}
		if err == nil { //没有解析成功，err==nil表示已经解析完了
			break
		}
